...
import { productpom } from '../pages/productpomextends.page';
let productPage: productpom;
test.beforeEach(async ({ page }) => {
    await page.goto('https://www.saucedemo.com/');
    productPage = new productpom(page); // 
    await productPage.loginWithCredentials("standard_user", "secret_sauce")
...
test('cartBtn pour naviguer aprés ajout', { tag: "@addedToCart" }, async ({ page }) => {
    ...
    await expect(productPage.elementspro.cartBadge()).toBeVisible();
    await expect(productPage.elementspro.cartBadge()).toHaveText('1');
